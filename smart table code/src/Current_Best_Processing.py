...
def find_endpoints(skel):
    ...
    return np.argwhere(neighbor_count == 11)  # center + 1 neighbor
...
def ExtractLeafTips(inDirectoryPath) -> str:
    ...
    for img_path in image_files:
        ...
        skeleton = pcv.morphology.skeletonize(mask=mask1l)
        prune1, _, _ = pcv.morphology.prune(skel_img=skeleton, size=100)
        ...
        print(
            f'Plant Size CIELAB: {pixl:6d} ({pixl/(640*480)*100:6.2f}%)',
            " "*5,
            f'Progress: ',
            f"{'*'*int(np.floor(img_count/img_tot*50)):<50}",  # progress bar: 50 total characters
            f'({img_count/img_tot*100:6.2f}%)'
        )
        ...
        hori = np.concatenate((hori, skeleton), axis=1)



        filename = os.path.basename(img_path)
        pcv.print_image(hori, rf'{outDirectoryPath}\{filename}')
        pcv.print_image(prune1, rf'{outDirectoryPathSkel}\{filename}')
